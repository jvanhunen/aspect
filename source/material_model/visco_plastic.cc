  ...
  namespace MaterialModel
  {
             ...
             MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      ...
      for (unsigned int i=0; i < in.n_evaluation_points(); ++i)
        {
          // First initiate reactions terms to default zero value:
          for (unsigned int c=0; c<in.composition[i].size(); ++c)
             out.reaction_terms[i][c] = 0.0;

          // Next compute the max melt fraction if applicable, so that it can be used in e.g. viscosity:
          if (this->introspection().compositional_name_exists("maximum_melt_fraction"))
            {    
              const unsigned int melt_index = this->introspection().compositional_index_for_name("maximum_melt_fraction");
          ...
          // Next compute the equation of state variables and thermodynamic properties
