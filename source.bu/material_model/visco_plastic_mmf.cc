...
#include <aspect/material_model/visco_plastic_mmf.h>
...
namespace aspect
{
  namespace MaterialModel
  {
      ...
      for (unsigned int i=0; i < in.temperature.size(); ++i)
        {
          ...
          // Compute changes in the compositional fields (i.e. the accumulated strain).
          for (unsigned int c=0; c<in.composition[i].size(); ++c)
            ...
            if (this->introspection().compositional_name_exists("maximum_melt_fraction"))
              {   
                ...
                const double melt_frac = melt_fraction (in.temperature[i], in.pressure[i], in.composition[i], in.position[i]);
    ...
    template <int dim>
    void
    ViscoPlasticMMF<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection ("Visco Plastic MMF");
        {
    ...
    template <int dim>
    void
    ViscoPlasticMMF<dim>::parse_parameters (ParameterHandler &prm)
    {
    ...
    template <int dim>
    void
    ViscoPlasticMMF<dim>::create_additional_named_outputs (MaterialModel::MaterialModelOutputs<dim> &out) const
    {
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(ViscoPlasticMMF,
                                   "visco plastic MMF",
                                   ...
                                   "\n\n "
                                   " Difference wirth viscoplastic material model: "
                                   " calculates and tracks maximum melt fraction."  )
  }
}
